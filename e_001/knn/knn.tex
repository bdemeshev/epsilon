...
\begin{document}
	...
	\begin{frontmatter}
		...
		\begin{aug}
			\author{\imya{Александра} \fam{Кузнецова}}%
			
			\runauthor{А.~Кузнецова}
...
Воспользуемся теперь пакетом \texttt{FNN} (\cite{RFNNpackage14}), предварительно проделав с~данными для тестовой выборки то же, что и~с~учебными, в~итоге получив два больших массива, \texttt{TRAIN} и~\texttt{TEST}, а~также два вектора ответов, \ENGs{\texttt{Trainlabels}} и~\ENGs{\texttt{Testlabels}}. 
...
Функция \ENGs{\texttt{evaluate\_weka\_classifier}} оценивает качество результатов и~показывает, какие значения были классифицированы правильно в~результате пятикратного разбиения выборки на кусочки и~последующего усреднения, а~какие "--- нет. 
