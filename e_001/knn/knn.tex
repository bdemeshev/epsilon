...
\AUTHOR{Александра Кузнецова}{НИУ ВШЭ, Москва.}
\SHORTAUTHOR{Александра Кузнецова}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in library(RWeka): there is no package called 'RWeka'}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}IBk"{}}}\begin{alltt}
\hlstd{classifier}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'classifier' not found}}\begin{alltt}
\hlkwd{evaluate_Weka_classifier}\hlstd{(classifier,} \hlkwc{numFolds} \hlstd{=} \hlnum{5}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}evaluate\_Weka\_classifier"{}}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in predict(classifier, newdata = test\_weka): object 'classifier' not found}}\begin{alltt}
\hlstd{errors} \hlkwb{<-} \hlstd{(}\hlkwd{sum}\hlstd{(predictions} \hlopt{!=} \hlstd{test_labels))}\hlopt{/}\hlstd{m}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'predictions' not found}}\begin{alltt}
\hlstd{errors}
\end{alltt}
\begin{verbatim}
## [1] 0.2
